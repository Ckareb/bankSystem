...
import accessDataBase.read.readCustomersDB;
...
public class menuGiveCustomers {
    public void menuGiveCustomers() {
        JFrame frame = new JFrame("Получение данных о клиенте");
        ...
        JLabel nameLabel = new JLabel("Серию и номер паспорта:");
        ...
        submitButton.addActionListener(e -> {
            ...
            while (number == null) {
                if (nameField.getText().matches("\\d{10}")) {
                    number = nameField.getText();
                } else {
                    JOptionPane.showMessageDialog(frame, "Серия и Номер паспорта должны содержать 10 цифр.");
                    return; // Возвращаемся, чтобы пользователь ввел корректные данные
                }
            }

            // Передаем значение в метод для получения данных
            StringBuilder result = new readCustomersDB().readCustomersDB(number);
            if (result == null) {
...
//        System.out.println("Если хотите увидеть весь список просто нажмите enter");
//        String seriesNumberPassport = new seriesNumberPassport().seriesNumberPassport();
//
//        new readCustomersDB().readCustomersDB(seriesNumberPassport);
    }
}
